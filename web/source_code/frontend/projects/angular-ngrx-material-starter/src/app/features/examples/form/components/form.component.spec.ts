...
describe('FormComponent', () => {
  ...
  const getSaveButton = () =>
    loader.getHarness(
      MatButtonHarness.with({ text: 'bizniz.examples.form.save' })
    );

  const getResetButton = async () =>
    loader.getHarness(
      MatButtonHarness.with({ text: 'bizniz.examples.form.reset' })
    );
  ...
  it('should save form', async () => {
    ...
    expect(biznizSpy.calls.mostRecent().args[0].type).toBe('[Form] Update');
    expect(biznizSpy.calls.mostRecent().args[0].form).toEqual({
      autosave: false,
  ...
  it('should reset form', async () => {
    ...
    expect(biznizSpy).toHaveBeenCalledTimes(1);
    expect(biznizSpy.calls.mostRecent().args[0].type).toBe('[Form] Reset');
