...
from libs.exceptions.auth_exceptions import UserDoesNotExistError, UserExistsError
...
class UserProfile(_CassUser):
    def __init__(self, *args):
        ...
        with get_session() as session:
            ...
            results = session.execute(f'''SELECT first_name, middle_name, last_name, suffix, gender, 
                                        ...
                                        FROM profiles WHERE username = '{encrypted_username}'
                                        ''')
    ...
    def has_permission(self, business_name: str, permissions_to_check: list = []) -> bool:
        my_permissions = []
        for p in permissions_to_check: my_permissions.append(f"{business_name}.{p}")
    ...
    def add_permission(self, business_name: str, permissions_to_add: list) -> bool:
        ...
        for permission in permissions_to_add:
            if f"{business_name}.{permission}" not in self.permissions: permissions_list.append(permission)
        
        formatted_permissions = "|".join(permissions_list)

        with get_session() as session:
            my_dict = {}
            my_dict[business_name] = f"{formatted_permissions}"
    ...
    def delete_permission(self, business_name: str, permissions_to_delete: list) -> bool:
        # get current permissions and store them in list
        ...
        for permission in permissions_to_delete:
            if f"{business_name}.{permission}" in self.permissions: permissions_list.remove(permission)
