...
from libs.cass_auth.applications.application_template_model import ApplicationTemplateModel
...
class BusinessConsumer:
    ...
    @staticmethod
    @authorized
    def get_business_profile(user: User, payload: dict, required_permissions: dict, producer):
        my_user = user.profile_user
        biz = BusinessModel().get(payload.get('businessName'))
        
        payload = {
            'uid': biz.id,
            'images': biz.serialized_images,
            'about': biz.about,
        ...
        return {'key': 'business', "command": "update_business_profile", "payload": payload }
        
    @staticmethod
    @authorized
    def get_business_data(user: User, payload: dict, required_permissions: dict):
        my_user = user.profile_user
        biz = BusinessModel().get(payload.get('businessName'))
        payload = {
            'uid': biz.id,
        ...
        return { 'key': 'business', 'command': 'update_business_data', 'payload': payload }        

    @staticmethod
    @authorized
    # @handleErrors
    def commit_business_profile(user: User, payload: dict, required_permissions: dict, producer):
        my_user = user.profile_user
        biz = BusinessModel().get(payload.get('businessName'))
        print (payload)
        update_args = {}
        if payload.get('images') is not None: update_args['images'] = payload.get('images')
        if payload.get('about') is not None: update_args['about'] = payload.get('about')
        if payload.get('addresses') is not None: update_args['addresses'] = payload.get('addresses')
        if payload.get('mcNumber') is not None: update_args['mc_number'] = payload.get('mcNumber')
        if payload.get('dotNumber') is not None: update_args['dot_number'] = payload.get('dotNumber')
        
        biz.update_info(**update_args)
        
        return {'key': 'profile', "command": "commit_business_profile_success", "payload": {
            }}
        
        # else:
        #     return {'key': 'profile', "command": "commit_business_profile_failed", "payload": {
        #         }}
    
    @staticmethod
    @authorized
    def create_application_template(user: User, payload: dict, required_permissions: dict, producer):
        my_user = user.profile_user
        app = ApplicationTemplateModel().create_from_dict(d=payload)
        return {'key': 'business', 'command': 'create_application_template_success', 'payload': {
            'message': 'success'
        }}
