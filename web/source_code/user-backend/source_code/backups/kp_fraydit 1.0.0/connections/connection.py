...
class ProducerNative(SerializingProducer):
    ...
    def produce(self, topic: str, values: list, keys: list) -> None: # Overrides parent
        ...
        while True:
            try:
                ...
                if e.args[0].retrievable():
                    ...
                    self.abort_transaction()
                    KafkaConnection.produce_queue.put([self.__topic_name, values, keys, self.__conf])
...
class KafkaConnection(metaclass=SingletonMeta):
    ...
    def __produce_record(self, thread_no: int, topic: str, values: list, keys: list, conf_: dict) -> None:
        ...
        print(f'Thread #{prod.tx_id} finished task #{thread_no} | {topic} | ')
