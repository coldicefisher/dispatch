...
avro_schema_field_types = ['null', 'boolean', 'long', 'float', 'double', 'string', 'byte[]']
...
class SchemaEngine(SchemaRegistryClient, metaclass=SingletonMeta):
    ...
    def validate_schema(self, subject_name: str, schema_name: str, base_schema_source: str) -> bool:
        '''
        Checks the schema in kafka against a base schema and adds any misisng fields or creates the schema altogether if missing.
        '''
        # VALIDATE VALUE SCHEMA ////////////////////////////////////////////////

        # Load the json data structure
        if base_schema_source[:4] == 'http':
            r = requests.get(base_schema_source)
            data = r.json()
            
        else:
            with open(base_schema_source) as json_file:
        ...
        return self.update_schema(new_schema_dict, schema_name)

    def update_schema(self, schema_dict: dict, schema_name: str, schema_type: str='AVRO') -> bool:
        formatted_schema_str = str(schema_dict).replace("'", '"') # replace single quotes with double quotes
        
        if schema_type == 'AVRO': new_schema = Schema(schema_str=str(formatted_schema_str),schema_type='AVRO')
        elif schema_type == 'JSON': new_schema = Schema(schema_str=str(formatted_schema_str),schema_type='JSON')
        else: return False
        print(new_schema.schema_str)
    ...
    def alter_field(self, schema_name: str, field_name: str, field_type: str, 
        ...
        if current_schema.schema_type == 'AVRO':        
            ...
            # Update the schema
            return self.update_schema(current_schema_dict, schema_name, 'AVRO')
    ...
    def delete_field(self, schema_name: str, field_name: str) -> bool:
        '''
        For whatever reason, schemas will not delete fields.
        '''
