...
# Test imports
from kp_fraydit import custom_types, custom_errors, datetime_functions, schema_client
from kp_fraydit.connections import connection
from kp_fraydit.producers import producer
...
def test_AvroAvro_producer_functionality():
    ...
    # Check value fields
    print (prod.value_field_names)
    print (prod.required_value_field_names)
    assert prod.value_field_names == ['open', 'high', 'low', 'close', 'volume', 'nullField', 'sourceNested', 'timestampNested']
    # Check required value fields
    assert prod.required_value_field_names == ['open', 'high', 'low', 'close', 'volume', 'sourceNested', 'timestampNested']
    # Check optional value fields
    assert prod.optional_value_field_names == ['nullField']
    # Check missing fields
    assert prod.missing_value_field_names == ['open', 'high', 'low', 'close', 'volume', 'sourceNested', 'timestampNested']
    # Check inclusive fields
    assert prod.inclusive_optional_value_field_names == []
    
    # Add optional inclusive field
    prod.inclusive_optional_value_field_names = ['nullField']
    ...
    # Check key fields
    assert prod.key_field_names == ['key','date','nullField']
    # Check required key fields
    assert prod.required_key_field_names == ['key', 'date']
    # Check optional key fields
    print (prod.key_field_names)
    print (prod.optional_key_field_names)
    assert prod.optional_key_field_names == ['nullField']
    ...
    prod.inclusive_optional_key_field_names = ['nullField']
...
def test_JsonJson_producer_functionality():
    ...
    print ('')
    print ('')
    prod = Producer('test_key_json_value_json')

    # Test that value fields
    assert prod.value_field_names == ['myField1', 'myField2', 'myField3']
    # Test required fields work
    assert prod.required_value_field_names == ['myField2', 'myField3']
    # Test optional fields work
    assert prod.optional_value_field_names == ['myField1']
    # Test missing fields work
    assert prod.missing_value_field_names == ['myField2', 'myField3']
    # Test inclusive fields work
    assert prod.inclusive_optional_value_field_names == []
    # Add optional inclusive field
    prod.inclusive_optional_value_field_names = ['myField1']
    ...
    # Test key fields
    assert sorted(prod.key_field_names) == ['myField1','myField2','myField3']
    # Test required key fields
    assert prod.required_key_field_names == ['myField3']
    # Test optional key fields
    assert prod.optional_key_field_names == ['myField1', 'myField2']
    # Test missing key fields
    assert prod.missing_key_field_names == ['myField3']
    print ('Key fields functioning...')
    print ('')
    
    prod.inclusive_optional_key_field_names = ['myField2']
...
def test_AvroNone_producer_functionality():
    ...
    assert sorted(prod.value_field_names) == ['myField1']
    # Test required fields work
    
    assert sorted(prod.required_value_field_names) == []
    # Test optional fields work
    assert prod.optional_value_field_names == ['myField1']
    # Test missing fields work
    assert sorted(prod.missing_value_field_names) == ['myField1']
    ...
    # Test key fields
    assert prod.key_field_names == ['key']
    # Test required key fields
    assert prod.required_key_field_names == []
    # Test optional key fields
    assert prod.optional_key_field_names == ['key']
    # Test missing key fields
    print ('Key fields functioning...')
    print ('')
    
    prod.inclusive_optional_key_field_names = ['key']
    # Test optional inclusive key fields after addition
...
def test_NoneJson_producer_functionality():
    ...
    prod = Producer('test_value_None')

    # Test that value fields

    assert prod.value_field_names == ['value']
    # Test required fields work
    assert prod.required_value_field_names == ['value']
    # Test optional fields work
    assert prod.optional_value_field_names == []
    # Test missing fields work
    ...
    assert sorted(prod.key_field_names) == ['myField1', 'myField2', 'myField3']
    # Test required key fields
    assert sorted(prod.required_key_field_names) == ['myField1', 'myField2']
    # Test optional key fields
    assert prod.optional_key_field_names == ['myField3']
    # Test missing key fields
    print ('Key fields functioning...')
    print ('')
    
    prod.inclusive_optional_key_field_names = ['myField3']
    # Test optional inclusive key fields after addition
...
def test_producer_no_schema():
    ...
    prod1 = Producer('test_no_schema')
    # Check the value fields
    assert prod1.value_field_names == ['value']
    assert prod1.required_value_field_names == ['value']
    assert prod1.optional_value_field_names == []
    # Cehck the key fields
    assert prod1.key_field_names == ['key']
    assert prod1.optional_key_field_names == ['key']
    assert prod1.required_key_field_names == []
    # Require key and check attributes
    assert prod1.missing_key_field_names == []
    assert prod1.missing_value_field_names == ['value']
    prod1.inclusive_optional_key_field_names = ['key']
    ...
    prod1.inclusive_optional_key_field_names = ['key']
    assert prod1.missing_key_field_names == ['key']
    assert prod1.inclusive_optional_key_field_names == ['key']
    assert prod1.optional_key_field_names == ['key']
    assert prod1.required_key_field_names == []
...
def test_producer_stress_test():
    ...
    prod_avro_avro = Producer('test')
    # prod_none_avro = Producer('test_key_None')
    prod_json_none = Producer('test_value_None') 
    prod_json_json = Producer('test_key_json_value_json')
    prod_none_none = Producer('test_no_schema')
    ...
    for i in range(r):
        ...
        if i == r//4: 
            ...
            print ('Broker still offline. Sleeping 10 seconds...')
            time.sleep(10)
            ...
            print ('Broker online. Sleeping 10 seconds...')
            time.sleep(10)
        ...
        if i % 2 == 0: # Add optional fields to even numbers
            ...
            prod_none_none.inclusive_optional_key_field_names = ['key']
            ...
            prod_none_none.inclusive_optional_key_field_names = []
            prod_none_none.addValueArgs(value=f'value {i}')            
            prod_none_none.addKeyArgs(key=f'key {i}')
