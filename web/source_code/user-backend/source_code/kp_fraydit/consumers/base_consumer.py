...
class BaseConsumer:
    ...
    @classmethod
    def from_topic(cls, topic_name,  group_id: str, on_message: object=None) -> object:
        value_schema = ProcessedSchema(SchemaEngine().get_latest_schema(f'{topic_name}-value'))
        key_schema = ProcessedSchema(SchemaEngine().get_latest_schema(f'{topic_name}-key'))
        return cls(topic_name, value_schema, key_schema, group_id, on_message)
        ...
        def poll_loop(self):
            while True: # infinite loop
                ...
                while True: # Loop until poll does not fail
                    ...
                    if self.__on_message is not None: self.__on_message(flatten_dict(msg.key()), flatten_dict(msg.value()))
